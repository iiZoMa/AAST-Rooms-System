...
import { Calendar, CheckCircle, XCircle, Clock3, AlertCircle, Trash2, Building, Edit3, Save, Check, X } from 'lucide-react';
...
const SecretaryDashboard = () => {
  ...
  // Find peer delegators
  const peerDelegators = activeDels.filter(d => d.roleGranted === 'employee' || d.roleGranted === 'secretary').map(d => d.delegatorId);
  ...
  const getMinDate = () => {
    ...
    minimum.setDate(minimum.getDate() + 2); // 48 hours in advance for Secretary natively
  ...
  const isBooked = bookings.some(b => 
    b.roomName === roomName && b.date === date && b.time === time && 
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: '2rem' }}>Secretary Room Booking Dashboard (Multipurpose Only).</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2rem', marginBottom: '3rem' }}>
        ...
        <div className="glass-panel" style={styles.formPanel}>
          <h3 style={{ borderBottom: '2px solid var(--secondary-color)', paddingBottom: '0.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Calendar size={20} /> Multipurpose Room Request
          </h3>
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              <label className="form-label">Multipurpose Room Location</label>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
              <div style={{ flex: 1 }}>
                <label className="form-label">Date {canBypassLeadTimes ? "(Bypass Override Active)" : "(Min. 48h)"}</label>
        ...
        <div style={styles.listPanel}>
          ...
          {myBookings.map(b => (
            <div key={b.id} className="glass-panel" style={{ padding: '1.5rem', marginBottom: '1rem' }}>
              ...
              <p style={{ margin: '0 0 0.5rem 0', fontSize: '0.9rem' }}>{b.date} at {b.time} - Multipurpose</p>
          ...
          {myBookings.length === 0 && <p style={{ color: '#888' }}>No previous applications.</p>}
        </div>
