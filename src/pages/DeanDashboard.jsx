...
const DeanDashboard = () => {
  ...
  const { bookings, updateBookingStatus, addNotification } = useBookings();
  ...
  const handleApproveUser = (userId) => {
    approveUser(userId);
    const approvedUser = users[userId];
    if (addNotification && approvedUser) {
      addNotification('faisal', `قام السيد العميد أكرم باعتماد حساب جديد للموظف: ${approvedUser.name}`);
    }
  };
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
            {pendingUsers.map(u => (
              <div key={u.id} className="glass-panel" style={{ borderLeft: '4px solid var(--danger)' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <button onClick={() => handleApproveUser(u.id)} className="btn btn-success" style={{ flex: 1, backgroundColor: 'var(--success)' }}>
